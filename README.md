...
Quorum check – The DFS verifies that at least 2 servers are available.
Version assignment – The primary’s current file version is read and incremented to obtain new_version.
Primary update – The primary Server applies the update (Server.apply_update), storing the new content and version.
...
Ask the primary server for the current version of the file.
If the client has a cache entry with the same version and the entry is still marked valid, it uses the cached content.
Otherwise, it fetches the latest file from the nearest available replica via DistributedFileSystem.read_file, stores it in the cache, and registers for invalidation at the primary server.
...
The DFS enforces quorum and commits the update on the primary and replicas.
The client then reads back the file’s metadata from the primary to discover the new version number.
